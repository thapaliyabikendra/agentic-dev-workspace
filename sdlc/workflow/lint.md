...
For a debt class whose violation corpus is too large to fix at once,
enforcement graduates through a **ratchet baseline** instead of a
big-bang sweep:

1. **Grandfather.** At a cutover date, commit the existing violations
   to a named, script-generated baseline file. The baseline is
   generated, never hand-edited; its header states the regeneration
   command. File format is project-tooling territory (record as a
   project ADR) — the engine specifies only the discipline.
2. **Hold.** New violations (not in the baseline) fail the class's
   mechanical twin (CI gate / `engine-lint` / `kb:trace`); they do not
   route to OQ — they are fixed before landing.
3. **Shrink.** The baseline only shrinks, never grows. Touching a
   baselined file obligates remediating its entry in the same change —
   debt retires one touched file at a time (strangler direction). When
   the baseline empties, the class enforces unconditionally and the
   baseline file is deleted.

**Enrollment.** A class is enrolled only when its baseline file is
committed; an unenrolled class routes findings per its own Action rule
above, unchanged. `prototype-drift` is the first enrollment candidate
(baseline generated by the ui repo's `kb:trace` against the unmapped /
drifted set); enroll others only when a concrete violation corpus
exists. Each cutover is recorded as a row in
[`grandfather-registry.md`](grandfather-registry.md).

This does not soften "The Lint Gate" anti-pattern: the ratchet's hold
step gates **new** violations mechanically at the class's existing
mechanical twin — it does not make the manual lint pass phase-blocking.
