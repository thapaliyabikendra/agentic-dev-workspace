...
| 2 | 2026-05-16 | Canonical per-type `log.md` retired (research + standards logs survive) | Pre-existing log entries preserved in git history | [`rule-history.md`](rule-history.md); CLAUDE.md HR-TIERED | None — permanent |
...
| 6 | 2026-05-17 | `id-claims.md` no longer takes `op: introduce` rows (R-NEW-9 amended) | Pre-cutover `op: introduce` rows are kept, not stripped | CLAUDE.md HR-ID; [`plan.md`](plan.md) | None — rows frozen in place |
