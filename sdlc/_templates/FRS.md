...
produces_nodes: []            # new DDD node IDs this FRS introduces *other than* FLW and ACT — covers ENT, CMD, STA, CON, INT, DEC, PERM, QRY plus any NDF-declared custom-type abbreviation registered in the target component's `node_definitions:` (per STD-007). Claimed at Phase 1, written to canonical at Phase 2 with status: proposed, flipped to active at Phase 3 merge. Phase 2 type-validity HARD-GATE rejects unknown type abbreviations.
