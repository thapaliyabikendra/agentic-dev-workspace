...
- **Retrieval is the token lever.** Skill compaction helps at the
  ...
  posture — live in [`sdlc/WORKFLOW.md → Retrieval discipline`](WORKFLOW.md).
...
- **❌ Silent node or ADR edits — no `index.md` re-sync, no `log.md` entry.** Indexes are a source of truth only as long as nothing slips past them.
  **✅ Run the lifecycle touch atomically (artifact + index + log).** If you can't write the log entry for an edit, the edit isn't ready. See [`workflow/maintenance-discipline.md`](workflow/maintenance-discipline.md).
...
- **❌ Copying `docs-backup/<file>.md` (or any legacy doc) directly into `docs/<topic>.md` or `docs/architecture.md`.** Bypasses the node + ADR + glossary discipline.
  **✅ Route through nodes + ADRs + glossary first; the derived report under `docs/overview/` comes after.** The legacy original stays in `docs-backup/` as audit trail.
