...
- **❌ Typing method bodies, brace bodies, SQL, or YAML in Phase 2 because the path looks "obvious."** Plans contain no syntax — you're over the line into Phase 3.
  **✅ Phase 1 names journey + modify-intent; Phase 2 names structures and enriches; Phase 3 writes them.** The full phase-birth schedule (which node types are born at which phase, with which status, and where) is procedural — canonical in [`CLAUDE.md` Rule 7](../CLAUDE.md#hard-rules) and the [`workflow/plan.md`](workflow/plan.md) HARD-GATE. This bullet is the *why*: structural names are auditable against the FRS; syntax is not.
