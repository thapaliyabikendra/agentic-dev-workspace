...
> `related: [CCC-008]` in the ADR's frontmatter. Cross-cutting governance
> rule: STD / ADR / CCC / DEC discriminator in
> [`../../../sdlc/workflow/authoring-adr.md`](../../../sdlc/workflow/authoring-adr.md).

## Baseline

Caching defaults use ABP distributed cache with per-operation opt-in (TBD —
ABP distributed cache; per-operation opt-in; cache key strategy and TTL
defaults to be fixed by the first FRS that opts into caching).
...
Each deviation ADR carries `related: [CCC-008]` in its frontmatter and a
