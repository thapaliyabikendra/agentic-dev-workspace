...
> `related: [CCC-003]` in the ADR's frontmatter. Cross-cutting governance
> rule: STD / ADR / CCC / DEC discriminator in
> [`../../../sdlc/workflow/authoring-adr.md`](../../../sdlc/workflow/authoring-adr.md).

## Baseline

Multi-tenancy defaults require that tenant scope is declared per operation
(TBD — tenant scope per operation: host | tenant | both, to be fixed by the
first FRS that touches a multi-tenant operation surface).
...
Each deviation ADR carries `related: [CCC-003]` in its frontmatter and a
