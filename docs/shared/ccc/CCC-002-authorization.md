...
> `related: [CCC-002]` in the ADR's frontmatter. Cross-cutting governance
> rule: STD / ADR / CCC / DEC discriminator in
> [`../../../sdlc/workflow/authoring-adr.md`](../../../sdlc/workflow/authoring-adr.md).

## Baseline

Authorization defaults are governed by the ABP permission system; permission
keys are defined per module (TBD — ABP permission system; permission keys per
module to be declared by the first FRS that touches the permission surface).
...
Each deviation ADR carries `related: [CCC-002]` in its frontmatter and a
